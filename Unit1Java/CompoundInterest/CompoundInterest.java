...
public class CompoundInterest {
    ...
    static double futureValue(double presentValue, double rate,
                              int targetYear) {
        return (presentValue*(Math.pow((1+(0.01*rate)), (numYears(targetYear)))));
    }
    ...
    static double futureValueReal(double presentValue, double rate,
        ...
        return (futureValue(presentValue, rate, targetYear)*
                (Math.pow((1-(0.01*inflationRate)),numYears(targetYear))));
    ...
    static double totalSavings(double perYear, int targetYear, double rate) {
        int k = 0;
        int i = numYears(targetYear);
        while (i>=0) {
            k += (perYear*Math.pow(1+(0.01*rate), i));
            i -= 1;}
        return k;
        }
    ...
    static double totalSavingsReal(double perYear, int targetYear, double rate,
         ...
         return (totalSavings(perYear, targetYear, rate))*(Math.pow(1-(0.01*inflationRate), numYears(targetYear)));
    }
